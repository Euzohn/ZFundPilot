...
- 📊 **实时估值**：交易日实时估算基金涨跌幅（AkShare fund_value_estimation_em），组合估算 P&L 一目了然，真实净值公布后估算自动失效
- 🔬 **基金对比**：多维度同框对比（基本信息、费率、规模、经理）+ 净值走势叠加 + 相关性矩阵，辅助选基决策
- 🔍 **基金筛选器**：从全市场基金池按类型/板块/关键词筛选候选基金，一键加入对比
...
- 📈 **定投策略回测**：指定基金 + 时间区间 + 定投频率（月/双周/周）+ 金额，用历史净值回测定投 vs 一次性投入，计算 XIRR 年化、最大回撤、夏普比率，计入申购费和赎回费
- 🔒 **API Key 加密存储**：AI 配置中的 API key 落盘自动加密（Fernet AES-128-CBC + HMAC-SHA256），主密钥独立存于 `data/secret.key`，旧版明文自动迁移
