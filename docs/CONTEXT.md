...
> 本文件不纳入 git 追踪（已在 .gitignore 中）。每次新对话开始时，Agent 应先读取此文件了解项目全貌。
...
本地运行的 Web 应用，核心功能：管理基金持仓 → 自动更新净值 → 计算收益与风险 → 提供结构优化建议。不是交易系统，不做自动买卖，不连接券商。
...
- 版本: `FastAPI(title="ZFundPilot API", version="0.5.0")`
...
- API: `GET /api/scheduler/status` + `PUT /api/scheduler/toggle`
...
- `Dockerfile`: 多阶段（node 22 构建前端 → python 3.11-slim 运行）
...
- 用户名 + 密码登录系统
- 涨跌颜色主题切换（国际/国内 A 股）
- 净值定时自动更新（APScheduler，工作日 21:00）
- AI 投顾对话（多会话、token 用量、联网搜索、持仓上下文）
...
- GitHub Release for v0.5.0（需 `gh auth login`）
- 其他页面 UI 改版（从首页 brutalist 风格延伸）
- 更多基金类型识别和板块分类
