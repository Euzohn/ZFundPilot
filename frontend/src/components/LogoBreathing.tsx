...
/** B2 金色呼吸 — 整体呼吸缩放 + 金环旋转 + Z 辉光脉动 + 花瓣明暗交替 */
export default function LogoBreathing({ className = "h-12 w-12" }: { className?: string }) {
  const id = useId()
  return (
    <svg viewBox="0 0 64 64" className={className} aria-label="加载中">
      <defs>
        <linearGradient id={`breathe-bg-${id}`} x1="0" y1="0" x2="1" y2="1">
          <stop offset="0%" stopColor="#0F1F3D" />
          <stop offset="100%" stopColor="#3B82F6" />
        </linearGradient>
      </defs>
      <g className="la-breathe-br">
        <rect width="64" height="64" rx="14" fill={`url(#breathe-bg-${id})`} />
        <g className="la-breathe-g12">
