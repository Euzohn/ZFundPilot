...
/** B6 罗盘摆动 — 花瓣像罗盘指针左右摇摆寻找方向 + Z 辉光 */
export default function LogoCompass({ className = "h-12 w-12" }: { className?: string }) {
  const id = useId()
  return (
    <svg viewBox="0 0 64 64" className={className} aria-label="加载中">
      <defs>
        <linearGradient id={`compass-bg-${id}`} x1="0" y1="0" x2="1" y2="1">
          <stop offset="0%" stopColor="#0F1F3D" />
          <stop offset="100%" stopColor="#3B82F6" />
        </linearGradient>
      </defs>
      <rect width="64" height="64" rx="14" fill={`url(#compass-bg-${id})`} />
      <g className="la-compass-w">
      ...
      <circle cx="32" cy="32" r="12.5" fill="none" stroke="#F59E0B" strokeWidth={0.8} className="la-compass-rp" />
      <g className="la-compass-z la-compass-zg">
