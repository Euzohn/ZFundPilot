...
<summary>更多截图（点击展开）</summary>
  ...
  <p><b>收益分析</b> — 浮动/已实现盈亏、XIRR 年化收益、组合收益曲线、基准对比、日/周/月/年日历视图</p>
  <br>
  <img src="assets/readme/screenshots_zh/risk.webp" alt="组合体检" width="90%">
  <p><b>组合体检</b> — 四维诊断评分（配置/风险/流动性/收益）+ 风险提示 + 结构优化建议</p>
  <br>
...
- **基金转换** — 一次录入转出基金（卖出）和转入基金（买入），原子创建两条关联交易，转出份额带持有量校验，赎回费/申购费分别自动计算，转换记录在流水中带标识，点击标识可跳转配对腿详情
...
- **收益分析** — 浮动盈亏、已实现盈亏、XIRR 年化收益（组合 + 单基金，现金流二分法）、组合收益曲线、基准对比（沪深300/上证指数/创业板指 + CPI 购买力/M2 财富排位水位线，数据持久化离线可用）、收益率排序（累计/年化切换）、日/周/月/年日历视图、按渠道堆叠柱状图
- **财富水位线（CPI/M2）** — 组合收益曲线叠加通胀购买力线（CPI 环比链乘价格水平）与社会财富排位线（M2 存量扩张），回答「扣掉通胀我真的变富了吗」。水位线以建仓日前最后月末为基期，建仓日精确为 0。AI 投顾上下文注入实际收益（扣通胀）与社会财富排位升降
...
- **定投计划自动执行** — 设置每日/每周/每双周/每月自动买入，遇非交易日顺延，自动计算手续费，T+1 回填净值。执行前校验基金申购状态（暂停申购/限购/低于起点自动跳过，次日重试）

### 组合体检与优化

- **组合体检** — 四维诊断评分（配置/风险/流动性/收益，每维 0-100）+ 综合定级（优秀→危险），复用风险指标 + 结构建议，一个 API 返回全量
